...
```{r}
#DEnuncias 2018
DEP18 <- c("Lima","Ancash","Cusco", "Arequipa","Libertad", "Junin","Ica","Piura","Callao","Lambayeque","Apurimac" )
CAN18 <- c(sum(DD18$DEPARTAMENTO == "LIMA"),sum(DD18$DEPARTAMENTO == "ANCASH"),sum(DD18$DEPARTAMENTO == "CUSCO"),sum(DD18$DEPARTAMENTO == "AREQUIPA"),sum(DD18$DEPARTAMENTO == "LA LIBERTAD"),sum(DD18$DEPARTAMENTO == "JUNIN"),sum(DD18$DEPARTAMENTO == "ICA"),sum(DD18$DEPARTAMENTO == "PIURA"),sum(DD18$DEPARTAMENTO == "CALLAO"),sum(DD18$DEPARTAMENTO == "LAMBAYEQUE"),sum(DD18$DEPARTAMENTO == "APURIMAC"))
Tabla18 <- data.frame(DEP18,CAN18)
barplot(Tabla18[1:20,2])
```


```{r}
table(DD18$DEPARTAMENTO)
```
